...
const CONTRACT_ADDRESS = import.meta.env.VITE_CONTRACT_ADDRESS || '0x8247B366cFCC30d9734258bF90cB8da80A9D7A92';
...
export function App() {
    ...
    const syncGrants = async (verbose: boolean = false) => {
      try {
        ...
        const onChainGrants: Grant[] = onChainData.map((c: any) => {
          ...
          let title = c.title || `On-Chain Initiative #${c.id}`;
          let propUrl = c.proposal_url;
          // Backward compatibility: handle legacy '|||' encoded URLs
          if (!c.title && c.proposal_url && c.proposal_url.includes('|||')) {
  ...
  const handleConnectWallet = async () => {
    ...
    if (window.ethereum) {
      try {
        ...
        // Force MetaMask to use the correct GenLayer Studionet configuration
        try {
          await eth.request({
          ...
          console.warn("Could not switch to GenLayer Studionet:", switchError);
        }
  ...
  const getGenLayerClient = () => {
    ...
    if (typeof window !== 'undefined' && window.ethereum) {
      ...
      const providerProxy = new Proxy(eth, {
        get(target: any, prop: string) {
          if (prop === 'request') {
            return async (req: { method: string, params?: any[] }) => {
              if (req.method === 'eth_sendTransaction' && req.params && req.params.length > 0) {
                const tx = req.params[0];
                // Strip EIP-1559 fields which cause connection drops on GenLayer RPC
                delete tx.maxFeePerGas;
                delete tx.maxPriorityFeePerGas;
                // Force Legacy transaction type by specifying gasPrice
                if (!tx.gasPrice) {
                  try {
                    tx.gasPrice = await target.request({ method: 'eth_gasPrice' });
                  } catch(e) {
                    tx.gasPrice = '0x0';
                  }
                }
                tx.type = '0x0'; // Explicitly set to Legacy
              }
              try {
                return await target.request(req);
              } catch (err: any) {
                console.error("Provider request failed:", req.method, err);
                throw err;
              }
            };
          }
  ...
  const handleDeployGrant = async (e: React.FormEvent) => {
    ...
    if (!newProposalUrl.startsWith("http://") && !newProposalUrl.startsWith("https://")) {
      alert("Proposal URL must be a valid HTTP/HTTPS URL so the AI can fetch it.");
      return;
    }
    ...
    try {
      ...
      if (errText.toLowerCase().includes("rate limited") || errText.includes("429") || errText.toLowerCase().includes("rate limit")) {
        setRpcError(`GenLayer Studionet RPC is currently rate limiting requests (500/hr limit exceeded). The network is congested.\n\nPlease try again later or wait for your IP limit to reset.`);
      } else {
        setRpcError(`Vault Deployment Failed on GenLayer:\n\n${errText}\n\n👉 Ensure your MetaMask is on Studionet and you have sufficient GEN for collateral & gas.`);
      }
    } finally {
  ...
  const handleSubmitEvidence = async (grantId: string, milestoneId: number) => {
    ...
    try {
      ...
      try {
        ...
        if (errorMsg.toLowerCase().includes("rate limited") || errorMsg.includes("429") || errorMsg.toLowerCase().includes("rate limit")) {
          setRpcError(`GenLayer Studionet RPC is currently rate limiting requests (500/hr limit exceeded). The network is congested.\n\nPlease try again later or wait for your IP limit to reset.`);
        } else {
          setRpcError(`On-Chain Evidence Submission Failed:\n\n${errorMsg}\n\n👉 Ensure your MetaMask wallet is connected to GenLayer Studionet and you confirm the real transaction.`);
        }
      ...
      if (errMsg.toLowerCase().includes("rate limited") || errMsg.includes("429") || errMsg.toLowerCase().includes("rate limit")) {
        setRpcError(`GenLayer Studionet RPC is currently rate limiting requests (500/hr limit exceeded). The network is congested.\n\nPlease try again later or wait for your IP limit to reset.`);
      } else {
        setRpcError(`Evidence submission error:\n\n${errMsg}`);
      }
    ...
    <div className="min-h-screen bg-[#07090f] text-zinc-100 flex flex-col font-sans antialiased selection:bg-cyan-500 selection:text-black relative">
      ...
      {rpcError && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="glass-panel rounded-xl p-8 border-red-500/40 max-w-md w-full flex flex-col items-center text-center shadow-2xl shadow-red-900/20 relative">
            <div className="w-12 h-12 rounded-full bg-red-500/20 flex items-center justify-center mb-4">
              <AlertTriangle size={24} className="text-red-400" />
            </div>
            <h2 className="text-xl font-bold text-white mb-2">Network Error</h2>
            <p className="text-sm text-gray-400 mb-6 whitespace-pre-wrap">{rpcError}</p>
            <button
              onClick={() => setRpcError(null)}
              className="bg-zinc-800 hover:bg-zinc-700 text-white px-6 py-2 rounded-lg text-sm font-semibold transition-colors border border-white/10 w-full"
            >
              Acknowledge
            </button>
          </div>
                    ...
                    >
                      ...
                      <div className="p-6 flex flex-wrap items-center justify-between gap-4 bg-[#0d111c]/95 border-b border-zinc-800/80">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          {ms.status === 'CUT' && (
                            <span className="px-4 py-2 rounded-xl text-xs font-mono font-extrabold uppercase bg-rose-500/20 text-rose-400 border border-rose-500/50 flex items-center shadow-md">
