...
class Contract(gl.Contract):
    ...
    def adjudicate_milestone(self, grant_id: str, milestone_id: str) -> str:
        ...
        if verdict == "RELEASE":
            ...
            ms.status = "APPROVED"
            emit_transfer(Address(grant.grantee), amount)
            ...
            ms.status = "PARTIAL"
            if half > bigint(0):
                emit_transfer(Address(grant.grantee), half)
            if rem > bigint(0):
                emit_transfer(Address(grant.funder), rem)
        elif verdict == "CUT":
            payout_amount = bigint(0)
            ms.status = "CUT"
            emit_transfer(Address(grant.funder), amount)
        else:
            verdict = "ESCALATE"
            ms.status = "ESCALATED"
